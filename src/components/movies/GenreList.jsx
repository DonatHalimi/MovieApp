...
const GenreList = () => {
    ...
    // Fetch all genres for movies
    useEffect(() => {
        const fetchGenres = async () => {
            try {
                const response = await fetch(`${VITE_BASE_URL}/genre/movie/list?api_key=${VITE_TMDB_API_KEY}`);
        ...
        // Call the function to fetch genres
        fetchGenres();
    }, []);

    return (
        <div className="container mx-auto pl-28 pr-28 pb-16 mt-20">
            <h3 className="text-2xl font-bold mb-6">Movie Genres</h3>
            ...
            <Splide
                ...
                {genres.map((genre) => (
                    <SplideSlide key={genre.id}>
                        <Link to={`/movies/genre/${genre.id}`} className="text-white">
