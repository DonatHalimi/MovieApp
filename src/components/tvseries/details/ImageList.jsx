...
const ImageList = ({ tvSeriesId }) => {
    const [images, setImages] = useState([]);

    // Fetch the images for a specific tv series
    useEffect(() => {
        const fetchImages = async () => {
            try {
                const response = await fetch(`${VITE_BASE_URL}/tv/${tvSeriesId}/images?api_key=${VITE_TMDB_API_KEY}`);
    ...
    return (
        <div className="container mx-auto pl-32 pr-32 pb-32 mt-1">
            ...
            {noImages ? (
                <p className="text-black">No image found for this TV Series.</p>
            ) : (
                <Splide options={{ perPage: 4, breakpoints: { 640: { perPage: 1 } } }}>
                    {images.map((image) => (
                        <SplideSlide key={image.file_path} className="relative group">
                            <img
