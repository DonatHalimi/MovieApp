...
const CreditList = ({ tvSeriesId }) => {
    const [credits, setCredits] = useState([]);
    const [showMore, setShowMore] = useState(false);

    // Fetch the credits for a specific tv series
    useEffect(() => {
        const fetchCredits = async () => {
            try {
                const response = await fetch(`${VITE_BASE_URL}/tv/${tvSeriesId}/credits?api_key=${VITE_TMDB_API_KEY}`);
        ...
        // Reset showMore when tvSeriesId changes
        setShowMore(false);
    ...
    return (
        <div className="container mx-auto pl-32 pr-32 pb-32">
            ...
            {noCredits ? (
                <p className="text-black">No credits found for this TV Series.</p>
            ) : (
